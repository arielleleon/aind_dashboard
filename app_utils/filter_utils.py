...
def apply_multi_select_filters(df: pd.DataFrame, filter_configs: Dict[str, Any]) -> pd.DataFrame:
    ...
    if stage_value:
        if isinstance(stage_value, list):
            filtered_df = filtered_df[filtered_df["current_stage_actual"].isin(stage_value)]
        else:
            filtered_df = filtered_df[filtered_df["current_stage_actual"] == stage_value]
    
    curriculum_value = filter_configs.get('curriculum')
    if curriculum_value:
        if isinstance(curriculum_value, list):
            filtered_df = filtered_df[filtered_df["curriculum_name"].isin(curriculum_value)]
        else:
            filtered_df = filtered_df[filtered_df["curriculum_name"] == curriculum_value]
...
def apply_alert_category_filter(df: pd.DataFrame, alert_category: str) -> pd.DataFrame:
    ...
    try:
        ...
        try:
            ...
            if hasattr(app_utils, 'alert_coordinator') and app_utils.alert_coordinator is not None:
...
def _apply_alert_category_filter_fallback(df: pd.DataFrame, alert_category: str) -> pd.DataFrame:
    """
...
def apply_all_filters(df: pd.DataFrame, time_window_value: int, stage_value: Any, 
                     curriculum_value: Any, rig_value: Any, trainer_value: Any, 
                     pi_value: Any, sort_option: str, alert_category: str, 
                     subject_id_value: Any) -> pd.DataFrame:
    """
    ...
    filtered_df = apply_multi_select_filters(filtered_df, {
        'stage': stage_value, 
