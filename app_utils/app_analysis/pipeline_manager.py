...
class DataPipelineManager:
    ...
    def initialize_reference_processor(
        ...
        logger.info(
            f"Reference processor initialized with {outlier_config['method']} outlier detection"
        )

        return self.reference_processor

    def initialize_quantile_analyzer(
        self, stratified_data: Dict[str, pd.DataFrame]
    ) -> QuantileAnalyzer:
        """
    ...
    def process_data_pipeline(
        ...
        """
        logger.info(
            f"Starting unified data processing pipeline with {len(df)} sessions"
        )
        ...
        if self.reference_processor is None:
            logger.info(
                "Initializing reference processor with default configuration..."
            )
        ...
        if "outlier_weight" in processed_df.columns:
            ...
            logger.info(
                f"{outlier_sessions}/{total_sessions} sessions ({outlier_rate:.1f}%) have outlier weights"
            )

        # Step 3: Prepare session-level data with rolling averages and strata assignments
        session_level_data = self.reference_processor.prepare_session_level_data(
            processed_df
        )
        ...
        session_with_percentiles = (
            self.quantile_analyzer.calculate_session_level_percentiles(
                session_level_data
            )
        )
        ...
        if self.cache_manager:
            ...
            if self.ui_data_manager:
                optimized_storage = self.ui_data_manager.optimize_session_data_storage(
                    comprehensive_data, cache_manager=self.cache_manager
                )
                ...
                logger.info(
                    f"Created optimized storage and UI structures for {len(optimized_storage['subjects'])} subjects"
                )

        logger.info(
            f"Unified pipeline complete: {len(comprehensive_data)} sessions processed"
        )
    ...
    def _add_session_metadata(self, session_data: pd.DataFrame) -> pd.DataFrame:
            ...
            if percentile_col in result_df.columns:
                ...
                if self.ui_data_manager:
                    result_df[category_col] = result_df[percentile_col].apply(
                        lambda x: (
                            self.ui_data_manager.map_percentile_to_category(x)
                            if not pd.isna(x)
                            else "NS"
                        )
                    )
                    ...
                    result_df[category_col] = result_df[percentile_col].apply(
                        lambda x: (
                            self._simple_percentile_to_category(x)
                            if not pd.isna(x)
                            else "NS"
                        )
                    )
        ...
        if "session_overall_percentile" in result_df.columns:
            if self.ui_data_manager:
                result_df["overall_percentile_category"] = result_df[
