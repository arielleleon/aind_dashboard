...
class EnhancedDataLoader:
    ...
    def load(self, load_bpod: bool = False) -> pd.DataFrame:
        ...
        """
        try:
            print(f"Loading session data with bpod={'enabled' if load_bpod else 'disabled'}")
    ...
    def reload_data(self, load_bpod: bool = False) -> pd.DataFrame:
        ...
        """
        print(f"Force reloading session data with bpod={'enabled' if load_bpod else 'disabled'}")
    ...
    def get_most_recent_subject_sessions(self) -> pd.DataFrame:
        ...
        try:
            ...
            sorted_data = all_data.sort_values(['subject_id', 'session_date'], 
    ...
    def get_sessions_count(self) -> Dict[str, int]:
        ...
        try:
            ...
            print(f"Session counts calculated: {len(session_counts)} subjects, {total_sessions} total sessions")
    ...
    def get_data_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"Data summary: {summary['total_sessions']} sessions, {summary['total_subjects']} subjects")
            return summary
    ...
    def validate_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'subject_id' in data.columns and 'session' in data.columns:
                ...
                if duplicates > 0:
                    issues.append(f"Found {duplicates} duplicate subject-session combinations")
            
            validation_result = {
                'valid': len(issues) == 0,
                'issues': issues,
                'total_sessions': len(data),
                'total_subjects': data['subject_id'].nunique() if 'subject_id' in data.columns else 0
            }
