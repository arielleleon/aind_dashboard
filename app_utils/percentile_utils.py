...
class PercentileCoordinator:
    ...
    def get_session_overall_percentiles(self, subject_ids: Optional[List[str]] = None, 
                                      use_cache: bool = True, 
        ...
        if 'subject_id' in session_data.columns and 'session_date' in session_data.columns:
    ...
    def _get_session_level_data(self, use_cache: bool = True) -> pd.DataFrame:
        ...
        if self.cache_manager.has('raw_data'):
            ...
            if raw_data is not None and not raw_data.empty:
                ...
                return self.pipeline_manager.process_data_pipeline(raw_data, use_cache=use_cache)
        
        # Return empty DataFrame if no data available
        return pd.DataFrame()
    
    def calculate_percentiles_for_sessions(self, session_data: pd.DataFrame, 
                                         feature_weights: Optional[Dict[str, float]] = None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            enhanced_data = self.percentile_calculator.calculate_session_overall_percentile(
                session_data
            )
            
            # Calculate rolling averages if available
            if hasattr(self.percentile_calculator, 'calculate_session_overall_rolling_average'):
    ...
    def get_percentiles_by_strata(self, strata_name: str, use_cache: bool = True) -> Optional[pd.DataFrame]:
        """
    ...
    def get_subject_percentile_history(self, subject_id: str, use_cache: bool = True) -> Optional[pd.DataFrame]:
        """
    ...
    def get_percentile_summary_stats(self, use_cache: bool = True) -> Dict[str, Any]:
        ...
        percentile_cols = [col for col in session_data.columns if 'percentile' in col.lower()]
    ...
    def validate_percentile_calculations(self, session_data: pd.DataFrame) -> Dict[str, Any]:
        """
