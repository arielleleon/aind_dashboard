...
class AppSubjectCompactInfo:
    ...
    def build(self, subject_id=None, app_utils=None):
        ...
        subject_display_data = app_utils.get_subject_display_data(
            subject_id, use_cache=True
        )
        ...
        if not subject_display_data:
            return html.Div(
                [
                    html.Div(
                        f"No data for {subject_id}", className="compact-subject-info"
                    )
                ],
                className="compact-info-section",
            )
        ...
        subject_table_data = next(
            (row for row in table_data if row["subject_id"] == subject_id), {}
        )
        ...
        return html.Div(info_components, className="compact-info-section")
