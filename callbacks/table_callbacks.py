    ...
    if callback_context.triggered and any(n_clicks_list or []):
        ...
        if "column-group-toggle" in triggered_prop_id and triggered_prop_id != ".":
            try:
                ...
                print(f"   Error parsing button ID: {triggered_prop_id}") # this is not working
                pass
...
@callback(Output("session-table", "columns"), [Input("column-groups-state", "data")])
def update_table_columns(column_groups_state):
    ...
    if not column_groups_state:
        ...
        expanded_groups = [
            group_id
            for group_id, is_expanded in column_groups_state.items()
            if is_expanded
        ]
    ...
    for col_id in visible_column_ids:
        if col_id in all_table_data.columns:
            column_def = {
                "name": formatted_column_names.get(
                    col_id, col_id.replace("_", " ").title()
                ),
                "id": col_id,
            }
